...
const generateQuestion = (difficulty: 'easy' | 'medium' | 'hard'): Question => {
  let num1: number, num2: number, optionsCount: number;
...
export default function MultiplicationGame67() {
  ...
  const moveToNextQuestion = () => {
    if (questionNumber >= TOTAL_QUESTIONS) {
      navigate('/game-6-7/result', { state: { correctCount, wrongCount, gameType: 'multiplication' } });
    } else {
          ...
          <div className={`grid gap-4 md:gap-6 ${currentQuestion.options.length <= 4 ? 'grid-cols-2' : 'grid-cols-3'}`}>
            {currentQuestion.options.map((option) => {
              ...
              if (selectedAnswer === option) {
                ...
                buttonClass += " bg-gradient-to-br from-purple-500 to-pink-500 opacity-50 cursor-not-allowed";
              } else {
                buttonClass += " bg-gradient-to-br from-purple-500 to-pink-500 hover:shadow-xl transform hover:scale-105";
              }
