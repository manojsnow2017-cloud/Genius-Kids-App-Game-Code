...
const generateQuestion = (difficulty: 'easy' | 'medium' | 'hard'): Question => {
  let num1: number, num2: number, maxSum: number, optionsCount: number;
...
export default function AdditionGame67() {
  ...
  const moveToNextQuestion = () => {
    if (questionNumber >= TOTAL_QUESTIONS) {
      navigate('/game-6-7/result', { state: { correctCount, wrongCount, gameType: 'addition' } });
    } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-400 via-teal-400 to-blue-500 p-4">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-12">
          ...
          <div className={`grid gap-4 md:gap-6 ${currentQuestion.options.length <= 4 ? 'grid-cols-2' : 'grid-cols-3'}`}>
            {currentQuestion.options.map((option) => {
              let buttonClass = "py-6 md:py-8 text-3xl md:text-4xl font-bold text-white rounded-2xl shadow-lg transition-all";
              ...
              if (selectedAnswer === option) {
                ...
                buttonClass += " bg-gradient-to-br from-green-500 to-teal-500 opacity-50 cursor-not-allowed";
              } else {
                buttonClass += " bg-gradient-to-br from-green-500 to-teal-500 hover:shadow-xl transform hover:scale-105";
              }
